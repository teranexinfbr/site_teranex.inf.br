<?php
/**
 * Backend para Formulário de Contato - Teranex
 *
 * Este script recebe dados via POST em formato JSON, valida as informações,
 * e envia um e-mail para o destinatário configurado.
 *
 * COMO USAR:
 * 1. Hospede este arquivo em um servidor com suporte a PHP.
 * 2. Configure a variável $destinatario com o seu e-mail.
 * 3. Atualize a URL no frontend React para apontar para este script.
 */

// --- CONFIGURAÇÃO DE CORS (Cross-Origin Resource Sharing) ---
// Permite que o seu frontend React (em outro domínio/porta) se comunique com este script.
// Em um ambiente de produção, é altamente recomendável restringir a origem.
// Exemplo: header("Access-Control-Allow-Origin: https://www.seusite.com");
...
// Limpa e sanitiza os dados para segurança.
...
$destinatario = "arthurjhonathaslima@gmail.com"; // ! IMPORTANTE: Substitua pelo seu e-mail!
...
// Cabeçalhos do e-mail para garantir o envio correto e a formatação.
...
if (mail($destinatario, $assunto, $corpo_email, $headers)) {
    ...
    echo json_encode(['status' => 'sucesso', 'mensagem' => 'Sua mensagem foi enviada com sucesso! Agradecemos o contato.']);
} else {
    // A função mail() pode falhar por configurações do servidor.
    http_response_code(500); // Internal Server Error
    echo json_encode(['status' => 'erro', 'mensagem' => 'Ocorreu um erro no servidor ao tentar enviar a mensagem. Por favor, tente novamente mais tarde.']);
}
?>
```

Também atualizei o código no Canvas para integrar o formulário com o backend. A função `handleSubmit` agora envia os dados para o script PHP e exibe as mensagens de sucesso ou erro retornadas por ele.
  ...
  const teamData = [
    {
      ...
      qualifications: ["Especialista em Redes Cisco (CCNA)", "Técnico em Manutenção de Hardware", "Certificação CompTIA A+"]
    },
    {
      name: "Mariana Oliveira",
      imageUrl: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=2070&auto=format&fit=crop",
      qualifications: ["Analista de Suporte Pleno", "Especialista em Sistemas Operacionais Windows e Linux", "ITIL Foundation"]
    },
    {
      name: "João Pereira",
      imageUrl: "https://images.unsplash.com/photo-1560250097-0b93528c311a?q=80&w=1887&auto=format&fit=crop",
      qualifications: ["Gerente de Infraestrutura", "Certificação em Segurança de Redes", "Microsoft Certified: Azure Administrator"]
    },
  ];
...
const ContactPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // ! IMPORTANTE: Substitua pela URL real do seu script PHP quando hospedado.
      const response = await fetch('https://seuservidor.com/enviar-email.php', {
        method: 'POST',
        headers: {
    ...
    <div>
      ...
      <div className="contact-container">
        <div className="contact-info">
          ...
          <div className="contact-info-item">
            <PhoneIcon />
            <a href="https://wa.me/5582988814747" target="_blank" rel="noopener noreferrer">(82) 9 8881-4747</a>
          </div>
        </div>
